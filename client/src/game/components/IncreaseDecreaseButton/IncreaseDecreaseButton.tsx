...
const IncreaseDecreaseButton = ({
  ...
  const handleMouseDown = (event: React.MouseEvent) => {
    ...
    const t = window.setInterval(() => fire(event, event.ctrlKey, event.altKey), 100)
    setTimer(t)
  }
  ...
  if (label)
  {
    content = <Button {...props} size="small">{label}</Button>
  }
  else
  {
    let icon
    ...
    content = <IconButton {...props} size="small">{icon}</IconButton>
  }
