...
import { PlanetID, IPlatoon, IPlatoonBasic, ShipID } from "../simulation/types.d"
import { Player } from "./Player"

export type { IPlatoon, IPlatoonBasic }

const sumPlatoonStrengths = (platoons: IPlatoonBasic[]) => platoons.reduce(( value, p, index, arr) => value + p.strength, 0)
...
export const PlatoonsOnPlanet = Recoil.selectorFamily<IPlatoonBasic[], { planet: PlanetID }>({
    key: "platoonsOnPlanet",
    get: ({ planet }) => ({ get }) => {
        const player = get(Player)
        const platoons = get(Platoons)
        return platoons.filter((p) => p.status === PlatoonStatus.Recruited && p.location.planet === planet && p.owner === player.id)
    }
})
...
export const PlanetEnemyStrength = Recoil.selectorFamily<number, { planet: PlanetID }>({
