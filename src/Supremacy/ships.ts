...
export const transitionShip = (
    ...
    if (!transitionMatrix[currentPosition].includes(targetPosition)) {
        console.error(`Cannot move ship ${ship.name} from ${currentPosition} to ${targetPosition}`)
    } else {
        let planet
        if (ship.position === "orbit" || ship.position === "docked" || ship.position === "surface") {
        ...
        try {
            ...
            switch (targetPosition) {
                case "docked": {
                    const dockedShips = ships.filter((ship) => isDocketAtPlanet(ship, planet))
                ...
                case "surface": {
                    const landedShips = ships.filter((ship) => isOnPlanetSurface(ship, planet))

                    if (planet.type === "lifeless") {
                        throw new Error("Cannot land a ship on a lifeless planet")
                    } else if (planet.owner !== player) {
                        throw new Error(`Player does not have permission to land a ship at planet ${planet.name}`)
                    } else if (landedShips.length >= 6) {
                        throw new Error(`Planet ${planet.name} surface locations are full`)
                    } else {
                        // Any ship can dock at any planet, assuming there is space
                        changes = {
                            position: "surface",
                ...
                case "orbit": {
                    ...
                    if (ship.position !== "docked") {
                        const maybePlanet = ship.position !== "outer-space" ? ship.location.planet : "-"
                        ...
                        throw new Error(`Cannot launch ship ${ship.name} without full crew`)
