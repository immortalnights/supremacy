...
export const canPurchaseShip = (
    ...
    const ownedAtmos = ships.filter((ship) => ship.class === "Atmosphere Processor" && ship.owner === player)
    ...
    if (dockedShips.length >= 3) {
        ...
        if (ships.length > 32) {
            console.error(`Player ${player} cannot own more than 32 ships`)
        } else {
            canPurchase = true
        }
    }
...
export const applyPurchaseShip = (
    ...
    const newShip = commissionShip(blueprint, name, date, modifiedPlanet, bayIndex)
