...
io.on("connection", (socket) => {
    ...
    socket.on("subscribe", (key, callback) => {
        const ok = manager.subscriptions.subscribe(
            player,
            key as SubscriptionType
        )
        callback(ok)
    })
    ...
    socket.on("unsubscribe", (key) => {
        ...
        manager.subscriptions.unsubscribe(player, key as SubscriptionType)
    })
    ...
    socket.on("player-create-room", (callback) => {
        ...
        manager.subscriptions.notify("rooms", "add", room)
    ...
    socket.on("player-join-room", (id, callback) => {
        ...
        const room = manager.rooms.find((room) => room.id === id)
        if (!room) {
    ...
    socket.on("player-join-game", (id, callback) => {
        ...
        const game = manager.games.find((game) => game.id === id)
        if (!game) {
    ...
    socket.on("disconnect", () => {
        ...
        manager.subscriptions.unsubscribe(player)
        // Remove the player from any room or game
        handleLeaveRoom()
        handleLeaveGame()

        // Remove the player entirely
        const index = manager.players.indexOf(player)
