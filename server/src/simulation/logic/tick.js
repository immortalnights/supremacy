...
const updateShip = (ship, shipOwner, planet, currentDate) => {
    if (ship.heading) {
        if (date.lessThen(ship.heading.arrival, currentDate)) {
            console.log(
                `Ship ${ship.name} has arrived at ${
                    ship.heading.to.name || ship.heading.to.id
                }`
            )
            ship = { ...ship }
            ship.location = {
            ...
            if (ship.type === "Atmosphere Processor") {
                if (!planet.habitable) {
                    ...
                    const completion = date.add(
                        date.fromDays(duration),
                        currentDate
                    )
                    ...
                    console.log(
                        `${
                            ship.name
                        } has started terraforming planet. Will complete by ${date.format(
                            completion
                        )}`
                    )
                }
        ...
        switch (ship.type) {
            case "Atmosphere Processor": {
                if (ship.location.state === "active" && ship.terraforming) {
                    if (
                        date.lessThen(ship.terraforming.completion, currentDate)
                    ) {
                        console.log(
                            `Terraforming of planet ${ship.location.planet} completed.`
                        )
...
const handleCombat = (planet, planetOwner, platoons, date) => {
    ...
    if (attackers.length > 0 && defenders.length > 0) {
        ...
        perPlatoon = Math.max(
            Math.floor(baseTroopReduction / attackers.length),
            1
        )
        ...
        perPlatoon = Math.max(
            Math.floor(baseTroopReduction / defenders.length),
            1
        )
...
const plantCaptured = (planet, platoonsOnPlanet) => {
    const hasDefenders = !!platoonsOnPlanet.find(
        (p) => p.owner === planet.owner
    )
    const hasAttackers = !!platoonsOnPlanet.find(
        (p) => p.owner !== planet.owner
    )
...
const updatePlanet = (planet, planetOwner, platoons, date) => {
    if (planet.type !== "Lifeless") {
        ...
        if (oddDate) {
            const taxPerPop =
                planet.creditsPerPop *
                planet.multipliers.resources.credits *
                planet.tax
        ...
        const foodConsumed =
            planet.population *
            (planet.foodPerPop * planet.multipliers.resources.food)
        ...
        planet.resources.foodChange = changeSymbol(
            planet.resources.food,
            planet.resources.previousFood,
        ...
        if (targetGrowth < planet.growthAdjustment) {
            planet.growthAdjustment =
                planet.growthAdjustment - Math.min(Math.abs(diff), 1)
        ...
        if (targetGrowth < planet.growth) {
            planet.growth = planet.growth - 0.5
        } else if (targetGrowth > planet.growth) {
            planet.growth = planet.growth + 0.5
        }

        planet.growthChange = changeSymbol(
            planet.growth,
            planet.previousGrowth,
            planet.growthChange
        )
        ...
        if (oddDate) {
            const populationGrowth =
                (planet.population / 4) * (planet.growth / 100)
...
const tick = (snapshot, set) => {
    ...
    const nextDate = updateDate(
        { ...snapshot.getLoadable(store.date).contents },
        set
    )
    ...
    const localPlayerLost = !planets.find(
        (p) => p.owner != null && p.owner === localPlayer.id
    )
    const remotePlayerLost = !planets.find(
        (p) => p.owner != null && p.owner === remotePlayer.id
    )
    ...
    if (localPlayerLost) {
        ...
        ships = ships.map((ship) => {
            ...
            if (ship.heading && ship.heading.to) {
                planetIndex = planets.findIndex(
                    (p) => p.id === ship.heading.to.id
                )
            ...
            const [updatedShip, updatedPlanet] = updateShip(
                ship,
                player,
                planets[planetIndex],
                nextDate
        ...
        planets = planets.map((planet) => {
            ...
            if (planet.habitable && planet.owner !== undefined) {
                ...
                ;[planet, modifiedPlatoons] = handleCombat(
                    planet,
                    player,
                    platoonsOnPlanet,
                    nextDate
                ...
                if (plantCaptured(planet, modifiedPlatoons)) {
                    const conqueror = modifiedPlatoons.find(
                        (p) => p.owner !== planet.owner
                    ).owner
                    console.log(
                        `Planet ${planet.name} (${planet.id}) has been conquered by ${conqueror}`
                    )

                    planet = { ...planet }
                    planet.owner = conqueror
                    // loose 25% of the population
                    planet.population = planet.population * 0.75
                ...
                if (platoon.commissioned === false) {
                    if (platoon.troops > 0 && platoon.calibre < 100) {
                        ...
                        if (platoon.troopChange > 0) {
                            ...
                            platoon.calibre =
                                platoon.calibre - platoon.troopChange
                        }
                    ...
                    // Disband
                    console.warn(
                        `Platoon ${platoon.id} has been defeated on planet ${platoon.location.planet}`
                    )
