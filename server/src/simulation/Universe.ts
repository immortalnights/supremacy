...
import {
    calculateEspionageMissionCost,
    generateMissionReport,
} from "./logic/espionage"
import { calculatePlatoonRecruitmentCost, ordinal } from "./logic/platoons"
import {
    UniverseEvent,
    load as loadUniverseEvents,
...
const planetLocationFull = (
    ...
    if (limit) {
        const count = ships.filter(
            (s) =>
                s.location.planet === planet && s.location.position === position
        ).length
        full = count >= limit
    }
...
export default class Universe implements IUniverse, IWorld {
    ...
    join(player: string, ai: boolean, replacedPlayer?: string) {
        let ok = false
        console.assert(
            this.players.length < MAXIMUM_PLAYERS,
            `Universe is full!`
        ) // Maximum players
        ...
        if (!replacedPlayer) {
            ...
            if (starbase) {
                ...
                if (ai) {
                    ...
                    const platoon = this.platoons.find(
                        (p) => p.owner === player /* && p.name === "1st" */
                    )
                    if (platoon) {
                        platoon.troops = 500
        ...
        console.log(
            `Player ${player} has joined the Universe`,
            this.players.length
        )
        return ok
    }
    ...
    leave(player: string) {
        ...
        if (index !== -1) {
            ...
            console.log(
                `Player ${player} has left the Universe`,
                this.players.length
            )
        }
    }
    ...
    transferOwnership(fromPlayerID: string, toPlayerID: string) {
        ...
        console.log(
            `Transferred items from '${fromPlayerID}' to '${toPlayerID}'`
        )
    }
    ...
    dispatch(
        ...
        const beginTravel = (
            ...
            console.debug(
                `Distance from ${origin.location} to ${origin.location} is ${distance} and required ${requiredFuels} fuel`
            )
        ...
        switch (action as PlayerGameAction) {
                ...
                if (body.id !== undefined) {
                    ...
                    if (planet) {
                        if (planet.type === PlanetType.Lifeless) {
                            const atmos = this.ships.find(
                                (s) =>
                                    s.owner === player &&
                                    s.type === "Atmosphere Processor"
                            )
                            if (atmos) {
                                const currentPlanet = findPlanet(
                                    atmos.location.planet!
                                ) as Planet
                ...
                if (body.id !== undefined && body.mission) {
                    ...
                    if (planet) {
                        if (planet.habitable && planet.owner !== player) {
                            const capital = this.planets.find(
                                (p) => p.capital === true && p.owner === player
                            )
                            if (capital) {
                                const cost = calculateEspionageMissionCost(
                                    body.mission
                                )
                                if (capital.resources.credits >= cost) {
                                    const strength = this.platoons
                                        .filter(
                                            (p) =>
                                                p.location.planet === planet.id
                                        )
                                        .reduce(
                                            (prev, item) =>
                                                prev + item.strength,
                                ...
                                reason =
                                    "Failed to find capital planet of player"
                            }
                        } else {
                ...
                if (body.id !== undefined && body.direction) {
                    ...
                    if (!planet) {
                        ...
                        resultData = {
                            world: {
                                planets: [
                                    { ...planet.toJSON(), aggression: change },
                                ],
                            },
                        }
                    }
            ...
            case "transfer-credits": {
                ...
                if (body.planet !== undefined) {
                    const sourcePlanet = this.planets.find(
                        (p) => p.id === body.planet
                    )
                    ...
                    if (sourcePlanet && sourcePlanet.owner === player) {
                        if (sourcePlanet.capital === false) {
                            ...
                            if (capital) {
                                capital.resources.credits +=
                                    sourcePlanet.resources.credits
                                sourcePlanet.resources.credits = 0
                                result = true
                                resultData = {
                                    world: { planets: [sourcePlanet.toJSON()] },
                                }
                            } else {
            ...
            case "ship-purchase": {
                ...
                if (body.id) {
                    const capital = this.planets.find(
                        (p) => p.owner === player && p.capital === true
                    )
                    if (capital) {
                        const staticData = this.getStaticData()
                        const ship = staticData.ships[
                            body.id as keyof typeof staticData.ships
                        ] as IStaticShipDetails
                        if (ship) {
                            const cost = {
                                credits: 0,
                                minerals: 0,
                            ...
                            if (
                                ship.type === "Atmosphere Processor" &&
                                false ===
                                    this.events.find(
                                        (e) =>
                                            e.id === "terraforming-breakthrough"
                                    )!.completed
                            ) {
                                reason =
                                    "Atmosphere Processor is not yet available"
                            } else if (
                                ship.limit !== undefined &&
                                this.ships.filter((s) => s.type === ship.type)
                                    .length >= ship.limit
                            ) {
                    ...
                    if (ship) {
                        if (
                            ...
                            if (ship.harvester) {
                                ...
                                if (
                                    ship.type === "Solar-Satellite Generator" &&
                ...
                if (body.id !== undefined && body.location !== undefined) {
                    ...
                    if (ship && ship.location.planet !== undefined) {
                        ...
                        if (currentPlanet && destinationPlanet) {
                            if (
                                ship.canTravel(currentPlanet, destinationPlanet)
                            ) {
                                if (
                                    beginTravel(
                                        ship,
                            ...
                            reason =
                                "Failed to find current or destination planet"
                        }
                    } else {
                        reason = "Invalid ship ID"
                    }
                    ...
                    if (index >= 0) {
                        ...
                        if (ship.location.position === "docking-bay") {
                            ...
                            if (planet.owner === ship.owner) {
                                const capital = this.planets.find(
                                    (p) =>
                                        p.owner === player && p.capital === true
                                ) as Planet
                                console.assert(
                                    capital,
                                    "Failed to find player capital"
                                )
                        ...
                        if (!capital) {
                            ...
                            if (body.amount !== undefined) {
                                ...
                                if (
                                    ...
                                    reason =
                                        "Insufficient available civilians on planet"
                                } else {
                                    capital.population += populationChange
                ...
                if (body.id !== undefined) {
                    ...
                    if (!platoon) {
                        ...
                        if (!capital) {
                            ...
                            const cost =
                                calculatePlatoonRecruitmentCost(platoon)
                            if (capital.resources.credits < cost) {
                                ...
                                platoon.recruit(
                                    capital.id,
                                    capital.getPlayerAggression(player)
                                )
            ...
            case "platoon-relocate": {
                ...
                if (body.id !== undefined && body.ship !== undefined) {
                    ...
                    if (
                        !platoon ||
                        platoon.owner !== player ||
                        platoon.status !== PlatoonStatus.Recruited
                    ) {
                        reason = "Invalid platoon"
                    } else if (
                        !ship ||
                        ship.owner !== player ||
                        ship.capacity.platoons === 0
                    ) {
                        reason = "Invalid ship"
                    } else {
                        if (body.direction === "load") {
                            const onboard = this.platoons.filter(
                                (p) => p.location.ship === ship.id
                            )
                            ...
                            if (platoon.location.ship !== ship.id) {
                                ...
                                platoon.aggression =
                                    planet.getPlayerAggression(player)
    ...
    simulate(delta: number, speed: GameSpeed = "normal"): void {
        ...
        this.events.forEach((event) => {
            if (false === event.completed && this.date > event.when!) {
                ...
                if (event.repeat) {
                    ...
                    console.log(
                        `Event '${event.name}' will occur again on ${event.when}`
                    )
                } else {
        ...
        const calculateResourceChange = (base: number, multiplier: number) =>
            (base * multiplier) / 2
            ...
            if (ship.task === "harvesting" && ship.harvester) {
                ...
                if (planet) {
                    ...
                    if (harvester.cooldown < 0) {
                        ...
                        if (harvester.resources.energy < 0) {
                            if (
                                ...
                                planet.resources.food +=
                                    calculateResourceChange(
                                        harvester.resources.food,
                                        multipliers.food
                                    )
                                planet.resources.minerals +=
                                    calculateResourceChange(
                                        harvester.resources.minerals,
                                        multipliers.minerals
                                    )
                                planet.resources.fuels +=
                                    calculateResourceChange(
                                        harvester.resources.fuels,
                                        multipliers.fuels
                                    )
                            } else {
                                ship.task = "idle"
                            ...
                            planet.resources.energy += calculateResourceChange(
                                harvester.resources.energy,
                                multipliers.energy
                            )
                        }
                ...
                if (ship.heading) {
                    if (this.date > ship.heading.arrival) {
                        ...
                        console.log(
                            `Ship ${ship.name} (${ship.id}) has arrived at ${planet.name} (${planet.id})`
                        )

                        if (
                            ship.type === "Atmosphere Processor" &&
                            planet.type === PlanetType.Lifeless &&
                            planet.terraforming === false
                        ) {
                            const endDate: StarDate =
                                this.date + planet.terraformDuration
                            ...
                            console.log(
                                `Begin terrforming planet ${planet.name}, will complete ${endDate}`
                            )
                        }
                    }
                } else {
                ...
                if (this.date > terraformer.end) {
                    ...
                    planet.claim(
                        ship.owner,
                        terraformer.planetName,
                        this.date,
                        false
                    )
                    console.log(
                        `Completed terraforming planet ${planet.name} (${planet.id}) by ${ship.owner}`
                    )
                }
            }
        })
        ...
        this.platoons.forEach((platoon) => {
            ...
            if (platoon.owner === player) {
                ...
                if (platoon.location.planet !== undefined) {
                    const planet = this.planets.find(
                        (p) => p.id === platoon.location.planet
                    )
                    if (planet && planet.owner === player) {
                        platoonData = {
                            id: platoon.id,
